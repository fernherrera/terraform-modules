...
locals {
  ...
  vm_name                  = lower(replace(replace(replace(var.virtual_machine_name, local.location, ""), "vm", ""), "--", ""))
  vm_linux_name            = "${local.vm_name}-vm-${local.location}"
  vm_windows_name          = replace(local.vm_name, "/[[:^alnum:]]/", "")
  pip_name                 = lower("${local.vm_name}-${local.location}-pip")
  nic_name                 = lower("${local.vm_name}-${local.location}-nic")
  ipconfig_name            = lower("${local.vm_name}-${local.location}-nic-ipconfig")
  proxigrp_name            = lower("${local.vm_name}-${local.location}-proxigrp")
  availability_set_name    = lower("${local.vm_name}-${local.location}-avail")
  nsg_name                 = lower("${local.vm_name}-${local.location}-nsg")
  os_disk_name             = lower("${local.vm_name}-${local.location}-osdisk")
  data_disk_name           = lower("${local.vm_name}-${local.location}-disk")
  az_mon_diag_setting_name = lower("${local.vm_name}-${local.location}-nsg-diag")
}
...
data "azurerm_virtual_network" "vnet" {
  ...
  resource_group_name = can(var.virtual_network_resource_group_name) ? var.virtual_network_resource_group_name : local.resource_group_name
}
...
resource "azurerm_public_ip" "pip" {
  ...
  name                = lower("${local.pip_name}-0${count.index + 1}")
  ...
  tags = merge({ "ResourceName" = lower("${local.pip_name}-0${count.index + 1}") }, var.tags, )
...
resource "azurerm_network_interface" "nic" {
  ...
  name                          = var.instances_count == 1 ? local.nic_name : "${format("%s-%s", local.nic_name, count.index + 1)}"
  resource_group_name           = local.resource_group_name
  location                      = local.location
  ...
  tags                          = merge({ "ResourceName" = var.instances_count == 1 ? local.nic_name : "${format("%s-%s", local.nic_name, count.index + 1)}" }, var.tags, )

  ip_configuration {
    name                          = format("%s-%s", local.ipconfig_name, count.index + 1)
    primary                       = true
...
resource "azurerm_proximity_placement_group" "appgrp" {
  ...
  name                = local.proxigrp_name
  resource_group_name = local.resource_group_name
  location            = local.location
  tags                = merge({ "ResourceName" = local.proxigrp_name }, var.tags, )
...
resource "azurerm_network_security_group" "nsg" {
  ...
  resource_group_name = local.resource_group_name
  location            = local.location
  tags                = merge({ "ResourceName" = local.nsg_name }, var.tags, )
...
resource "azurerm_linux_virtual_machine" "linux_vm" {
  ...
  name                            = var.instances_count == 1 ? substr(local.vm_linux_name, 0, 64) : substr(format("%s%s", local.vm_linux_name, count.index + 1), 0, 64)
  resource_group_name             = local.resource_group_name
  location                        = local.location
  ...
  tags                            = merge({ "ResourceName" = var.instances_count == 1 ? substr(local.vm_linux_name, 0, 64) : substr(format("%s%s", local.vm_linux_name, count.index + 1), 0, 64) }, var.tags, )
...
resource "azurerm_windows_virtual_machine" "win_vm" {
  ...
  name                         = var.instances_count == 1 ? substr(local.vm_windows_name, 0, 15) : substr(format("%s%s", local.vm_windows_name, count.index + 1), 0, 15)
  computer_name                = var.instances_count == 1 ? substr(local.vm_windows_name, 0, 15) : substr(format("%s%s", local.vm_windows_name, count.index + 1), 0, 15)
  resource_group_name          = local.resource_group_name
  location                     = local.location
  size                         = var.virtual_machine_size
  admin_username               = var.admin_username
  ...
  tags                         = merge({ "ResourceName" = var.instances_count == 1 ? substr(local.vm_windows_name, 0, 15) : substr(format("%s%s", local.vm_windows_name, count.index + 1), 0, 15) }, var.tags, )
...
resource "azurerm_managed_disk" "data_disk" {
  ...
  name                 = "${local.data_disk_name}-${each.value.idx}"
  resource_group_name  = local.resource_group_name
  location             = local.location
  storage_account_type = lookup(each.value.data_disk, "storage_account_type", "StandardSSD_LRS")
  create_option        = "Empty"
  disk_size_gb         = each.value.data_disk.disk_size_gb
  tags                 = merge({ "ResourceName" = "${local.data_disk_name}-${each.value.idx}" }, var.tags, )
...
resource "azurerm_monitor_diagnostic_setting" "nsg" {
  ...
  name                       = local.az_mon_diag_setting_name
